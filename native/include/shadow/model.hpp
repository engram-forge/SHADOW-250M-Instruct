...
enum class WeightKind : std::uint32_t { dense_f32 = 0, rvq = 1, ternary2 = 3, ternary3 = 4, dense_f16 = 5 };
...
public:
    ...
    void logits_into(std::span<const float> projected, std::span<float> output) const;
...
class Runtime {
public:
    Runtime(const std::filesystem::path& model, const std::filesystem::path& table);
    ~Runtime();
    GenerationStats generate(std::span<const std::uint32_t> prompt, const GenerationOptions& options);
