...
def run(args, tokens, group, output, candidate=False):
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--kernel",required=True);p.add_argument("--model",required=True);p.add_argument("--table",required=True);p.add_argument("--fixture",required=True);p.add_argument("--group",type=int);p.add_argument("--candidate-env",action="append",default=[]);p.add_argument("--limit",type=int,default=25);p.add_argument("--generate",type=int,default=33);p.add_argument("--threads",type=int,default=1);p.add_argument("--out",required=True);a=p.parse_args()
    if not a.group and not a.candidate_env: p.error("provide --group or --candidate-env")
    ...
    with tempfile.TemporaryDirectory(prefix="shadow-dotprod-") as d:
      d=pathlib.Path(d)
      for case in cases:
        bt,bl=run(a,case["tokens"],0,d/"base.npy");ct,cl=run(a,case["tokens"],a.group,d/"candidate.npy",True)
    ...
    summary={"cases":len(rows),"group":a.group,"candidate_env":a.candidate_env,"identical_rate":sum(r["tokens_equal"] for r in rows)/len(rows),"first_argmax_rate":sum(r["first_argmax_match"] for r in rows)/len(rows),"median_prefix":float(np.median([r["prefix"] for r in rows])),"median_first_rmse":float(np.median([r["first_rmse"] for r in rows])),"median_top10_overlap":float(np.median([r["first_top10_overlap"] for r in rows]))}
