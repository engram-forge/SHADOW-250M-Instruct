...
def test_tilelang_greedy_generation_crosses_split_attention_boundary(boundary_index):
    from pathlib import Path
    ...
    with torch.inference_mode():
        ...
        try:
            boundary = ATTENTION_PARALLELISM[boundary_index][0]
            previous_parallelism = (
                0 if boundary_index == 0
                else ATTENTION_PARALLELISM[boundary_index - 1][1]
            )
            next_parallelism = ATTENTION_PARALLELISM[boundary_index][1]
...
def test_tilelang_packed_fingerprint_logits_match_dense_reference():
    ...
    torch.testing.assert_close(actual_words, expected, rtol=1e-6, atol=1e-6)
    assert int(actual_words.argmax()) == int(actual.argmax())
    ...
    stored_token = torch.full_like(selected_token, -1)
    stored_position = torch.zeros(1, device="cuda", dtype=torch.int32)
    compile_candidate_argmax(
        block_values.numel(), vocabulary, store_output=True,
        advance_position=True,
    )(block_values, block_indices, stored_token, stored_position)
    ...
    assert int(stored_token) == int(selected_token)
    assert int(stored_position) == 1
    ...
    torch.testing.assert_close(word_values, block_values, rtol=1e-6, atol=1e-6)
...
def test_tilelang_double_rms_norm_is_bit_exact():
    from shadow_tilelang.engine import _rms_norm
    from shadow_tilelang.kernels import compile_double_rms_norm

    torch.manual_seed(450)
    x = torch.randn(1536, device="cuda", dtype=torch.bfloat16)
    first_weight = torch.randn(1536, device="cuda")
    second_weight = torch.randn(1536, device="cuda")
    actual = compile_double_rms_norm(1536)(x, first_weight, second_weight)
    expected = _rms_norm(_rms_norm(x, first_weight), second_weight)
    torch.testing.assert_close(actual, expected, rtol=0, atol=0)
