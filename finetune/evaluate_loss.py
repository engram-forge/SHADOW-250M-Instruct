...
def main():
    ...
    losses = []; tokens = 0; mtp_loss_sum=mtp_correct=0.0; mtp_tokens=0
    with torch.no_grad():
        for _ in range(args.batches):
            ...
            if args.evaluate_mtp:
                ...
                mtp_loss_sum+=float(metrics["mtp_loss"]*metrics["mtp_tokens"])
                mtp_correct+=float(metrics["mtp_accuracy"]*metrics["mtp_tokens"])
                mtp_tokens+=metrics["mtp_tokens"]
    ...
    if args.evaluate_mtp:
        result+=(f" mtp_loss={mtp_loss_sum/max(1,mtp_tokens):.6f}"
                 f" mtp_accuracy={mtp_correct/max(1,mtp_tokens):.6f}")
    print(result)
